...
if [[ "$HOSTNAME" == "$CONTROL_PLANE_0" ]]; then
    ...
    echo "kubectl label node <worker-node-name> node-role.kubernetes.io/worker=""" >> /root/kubeadm_join_cmd.sh
...
# 1️⃣ Enable kubectl autocompletion
source <(kubectl completion bash)

# 2️⃣ Make autocompletion permanent
echo 'source <(kubectl completion bash)' >> ~/.bashrc

# 3️⃣ Create alias "k" for kubectl
echo 'alias k=kubectl' >> ~/.bashrc

# 4️⃣ Enable autocompletion for "k" alias
...
# 1️⃣ Get the latest etcd version
ETCD_VERSION=$(curl -s https://api.github.com/repos/etcd-io/etcd/releases/latest | grep tag_name | cut -d '"' -f 4 | sed 's/v//')

# 2️⃣ Download the etcd binary
...
# 4️⃣ Move etcdctl to a system-wide location
sudo mv etcd-v${ETCD_VERSION}-linux-amd64/etcdctl /usr/local/bin/

# 5️⃣ Ensure correct permissions
sudo chmod +x /usr/local/bin/etcdctl

# 6️⃣ Verify installation
etcdctl version

# 7️⃣ Clean up the downloaded files
